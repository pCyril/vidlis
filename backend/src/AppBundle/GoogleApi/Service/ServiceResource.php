 ...
 */
namespace AppBundle\GoogleApi\Service;

use AppBundle\GoogleApi\Io\HttpRequest;
use AppBundle\GoogleApi\Io\REST;
use AppBundle\GoogleApi\Client;
use AppBundle\GoogleApi\Config;
...
class ServiceResource {
   ...
   * @throws \AppBundle\GoogleApi\Exception
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new \AppBundle\GoogleApi\Exception("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new \AppBundle\GoogleApi\Exception("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new \AppBundle\GoogleApi\Exception("($name) missing required param: '$paramName'");
        }
